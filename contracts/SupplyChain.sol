...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  ...
  modifier forSale (uint _sku) {
    ...
    require(_item.state == State.ForSale, "Item not marked for sale");
    require(_item.price > 0, "Invalid Price");
    _;
  }

  modifier sold (uint _sku) {
    Item storage _item = items[_sku];
    require(_item.state == State.Sold, "Item not marked as sold");
    require(_item.price > 0, "Invalid Price");
    _;
  }
  modifier shipped (uint _sku) {
    Item storage _item = items[_sku];
    require(_item.state == State.Shipped, "Item not marked as shipped");
    require(_item.price > 0, "Invalid Price");
    _;
  }
  ...
  modifier received (uint _sku) {
    Item storage _item = items[_sku];
    require(_item.state == State.Received, "Item not marked as received");
    require(_item.price > 0, "Invalid Price");
    _;
  }
